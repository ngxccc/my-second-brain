...
3. **Leaf Nodes (Nút lá)**:
   ...
   * Được liên kết vật lý với nhau bằng **Doubly Linked List** (Danh sách liên kết đôi). Khi thực hiện Range Query (`WHERE price BETWEEN 100 AND 500`), database chỉ cần dùng cây để định vị nút lá chứa `100`, rồi đi theo con trỏ `next` để quét ngang qua các nút lá kế tiếp đến khi gặp `500` $\rightarrow$ chỉ tốn đúng 1 lần đọc đĩa cho mỗi page kế tiếp mà không phải duyệt ngược lên các nút cha.
...
export const products = snakeCase.table("product", {
  id: uuid().primaryKey(),
